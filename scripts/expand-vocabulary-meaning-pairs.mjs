...
const files = ["src/vocabulary-basic.ts", "src/vocabulary-intermediate.ts", "src/vocabulary-advanced.ts"];
...
const hasEnglishPair = (value) => !/\brelated\b/.test(value) && /[,;/]/.test(value);
const hasChinesePair = (value) => !/相关/.test(value) && /[，,、；;]/.test(value);

function expandEnglish(value) {
  value = value.replace(/,\s*related\s+(.+)$/, "");
  if (hasEnglishPair(value)) return value;
...
function expandChinese(value) {
  ...
  if (hasChinesePair(value)) return value;
...
for (const file of files) {
  ...
  const updated = source.replace(
    /meanings: \{ English: "([^"]+)", "Simplified Chinese": "([^"]+)" \}/g,
    (_match, en, zh) => `meanings: { English: "${expandEnglish(en)}", "Simplified Chinese": "${expandChinese(zh)}" }`
  );
